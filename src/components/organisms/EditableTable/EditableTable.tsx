...
const toColumns = <T extends ObjectType>(columnConfig: EditableTableColumnConfig<T>, langCode: LangCode): Column<T>[] =>
  toPairs(columnConfig).reduce(
    (acc, [key, config]) => [
      ...acc,
      {
        ...config,
        field: key,
        title: config.title[langCode] ?? config.title.jp,
      },
    ],
    [] as Column<T>[],
  );
...
const Component = <T extends ObjectType>(props: EditableTableProps<T>) => {
  const langCode = useSelector(langCodeSelector, shallowEqual);
  const { data, columnConfig, onRowAdd, onRowUpdate, onRowDelete } = props;
  const title = useMemo(() => <I18nText langCode={langCode} i18nObj={props.title} />, [props.title, langCode]);
  const columns = useMemo(() => toColumns(columnConfig, langCode), [columnConfig, langCode]);
